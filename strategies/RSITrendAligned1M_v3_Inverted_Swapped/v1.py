...
class RSITrendAligned1M_v3_Inverted_Swapped(Strategy):
    """
    Author: MCP Client
    Description: INVERTED version of RSITrendAligned1M_v3 with SWAPPED distances.
                 
                 Inversion Logic:
                 - Original Buy becomes Inverted Sell
                 - Original Sell becomes Inverted Buy
                 - SL/TP DISTANCES swapped (not values)
                 
                 Example:
                 Original: Buy at 3515, SL 3510 (-5), TP 3525 (+10)
                 Inverted: Sell at 3515, SL 3520 (+5), TP 3505 (-10)
                 
                 This preserves the original RRR profile (1:2.5) but inverted
    """
    ...
    def risk_model(self, df: pd.DataFrame, vars: dict[str, pd.Series]) -> dict[str, pd.Series]:
        """
        SWAPPED DISTANCES:
        - Original SL distance = 2.5x ATR
        - Original TP distance = 6.25x ATR
        
        For INVERTED trade, we swap:
        - Inverted SL uses original TP distance = 6.25x ATR (but in opposite direction)
        - Inverted TP uses original SL distance = 2.5x ATR (but in opposite direction)
        
        This creates negative RRR (1:0.4), which we want to test
        """
        original_sl_distance = vars['atr'] * 2.5
        original_tp_distance = vars['atr'] * 6.25
        
        # SWAPPED: SL is further out (original TP distance), TP is closer (original SL distance)
