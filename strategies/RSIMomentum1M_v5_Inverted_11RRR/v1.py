...
class RSIMomentum1M_v5_Inverted_11RRR(Strategy):
    """
    Author: MCP Client
    Description: INVERTED version of RSIMomentum1M_v5 with 1:1 RRR.
    """
    ...
    def entry_long(self, df: pd.DataFrame, vars: dict[str, pd.Series]) -> pd.Series:
        # Original short entries -> now long
    ...
    def entry_short(self, df: pd.DataFrame, vars: dict[str, pd.Series]) -> pd.Series:
        # Original long entries -> now short
